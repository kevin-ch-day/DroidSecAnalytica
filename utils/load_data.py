...
def load_data_from_files(files):
    ...
    for fIndex in files:
        try:
            ...
            if data:
                insert_data_into_db(fIndex, data)
            else:
                logging.warning(f"No valid data parsed from {fIndex}.")
        except Exception as e:
            logging.error(f"Error in processing file {fIndex}: {e}")

def insert_data_into_db(file_path, data):
    """ Insert parsed data into the database. """
    sql_insert_data = """
        INSERT INTO android_malware_hashes 
        (malware_name_1, malware_name_2, md5, sha1, sha256, location, month, year)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    for record in data:
        try:
            dbu.run_sql(sql_insert_data, record)
        except Exception as e:
