...
import calendar
import re
from . import hash_utils, logging_utils

def check_files(file_paths):
    valid_files = []
    for file_path in file_paths:
        if not os.path.isfile(file_path):
            logging_utils.log_warning(f"File not found: {file_path}")
            continue
        if os.path.getsize(file_path) == 0:
            logging_utils.log_warning(f"File is empty: {file_path}")
            continue
...
def load_android_malware_hash_data():
    try:
        ...
        if not os.path.isdir(input_directory):
            logging_utils.log_error(f"Input directory '{input_directory}' not found.")
            return

        valid_files = check_files(files_to_parse)
        if not valid_files:
            logging_utils.log_error("No valid data files found for processing.")
            return

        load_data_from_files(valid_files)
        logging_utils.log_info(f"Data processing completed.")

    except Exception as e:
        logging_utils.log_error(f"Error during data processing: {e}")
...
def load_data_from_files(files):
    for file_path in files:
        try:
            ...
            logging_utils.log_error(f"Error in processing file {file_path}: {e}")
...
def parse_file(file_path):
    ...
    try:
        ...
        logging_utils.log_error(f'Error reading {file_path}: {e}')
        return []
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if line.startswith('**'):
            ...
            try:
                ...
                logging_utils.log_error(f"Error at line {line_number} in file {file_path}: {e}")
                logging_utils.log_error(f"Problematic data: '{line}'")

    if not malware_data:
        logging_utils.log_warning(f'No valid malware data extracted from file: {file_path}')
