...
def create_android_malware_hash_table():
    try:
        with database_manager.create_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
...
def view_android_hash_table_summary():
    sql = "SELECT COUNT(*) FROM android_malware_hashes"
    result = database_manager.execute_query(sql)
    if result:
...
def insert_data_into_malware_hashes(file_path, data):
    ...
    for record in data:
        try:
            database_manager.run_sql(sql_insert_data, record)
        except Exception as e:
