...
def check_for_table(table_name):
    try:
        ...
        if conn:
            result = database_manager.execute_sql(conn, f"SELECT 1 FROM {table_name} LIMIT 1", fetch=True)
            return bool(result)
...
def create_apk_record(conn, filename, filesize, md5, sha1, sha256):
    ...
    return database_manager.execute_sql(conn, sql, val)
...
def check_android_malware_hash_table_exists():
    try:
        conn = database_manager.connect_to_database()
        if conn:
            result = database_manager.check_for_table(conn, 'android_malware_hashes')
            return result
        return False
    except Exception as e:
...
def create_android_malware_hash_table():
    try:
        ...
        if conn:
            ...
            '''
            if database_manager.execute_sql(conn, sql_create_table):
                return True
            else:
